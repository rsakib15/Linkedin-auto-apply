...
class LinkedinEasyApply:
    ...
    def apply_jobs(self, location):
        ...
        for job_tile in job_list:
            ...
            if company.lower() not in [word.lower() for word in self.company_blacklist] and contains_blacklisted_keywords is False and link not in self.seen_jobs:
                try:
    ...
    def send_resume(self):
        try:
            previous_resume = self.browser.find_element(By.CLASS_NAME, "jobs-resume-picker__resume-list")
            if previous_resume:
                print("Sending Previous Resume")
                choose_resume = self.browser.find_element(By.CLASS_NAME, "jobs-resume-picker__resume-list").find_element(By.CLASS_NAME, "artdeco-button")
                choose_resume.click()
                print("Resume Sent")
            else:
    ...
    def fill_up(self):
        try:
            ...
            b4 = easy_apply_content.find_elements(By.CLASS_NAME,'pb4')
    ...
    def next_job_page(self, position, location, job_page):
        self.browser.get("https://www.linkedin.com/jobs/search/" + self.base_search_url + location + "&start=" + str(job_page*25))
        self.avoid_lock()
